...
export const metadata = {
  title: "Graph — Paul Graham · Investor Minds",
  description: "Knowledge graph of Paul Graham's ideas — 650+ concepts connected by shared themes.",
};
...
async function buildGraphData(): Promise<GraphData> {
  ...
  // Build slug → id map and count incoming links per node
    ...
    <div className="flex h-screen flex-col bg-[#0f0f0f]">
      ...
      <header className="flex shrink-0 items-center justify-between border-b border-white/10 px-5 py-3">
        <div className="flex items-center gap-3">
          <Link href="/paul-graham" className="text-sm text-white/40 hover:text-white/70 transition">
            ← Paul Graham
          </Link>
      ...
      <div className="flex-1 overflow-hidden">
        <KnowledgeGraph data={data} mindSlug="paul-graham" />
      </div>
    </div>
  );
}
